...
public class DragResizer {
    ...
    private static final OnDragResizeEventListener defaultListener = new OnDragResizeEventListener() {
        ...
        private void setNodeSize(Node node, double x, double y, double h, double w) {
            ...
            if (node instanceof Canvas) {
                ...
                ((Label) node).setPrefWidth(w);
                ((Label) node).setPrefHeight(h);
            }
            else if (node instanceof MediaView) {
                ((MediaView) node).setFitWidth(w);
                ((MediaView) node).setFitHeight(h);
            }
            else if (node instanceof WebView) {
                ((WebView) node).setPrefWidth(w);
                ((WebView) node).setPrefHeight(h);
            }
        }
    };
    ...
    public DragResizer(Node node, OnDragResizeEventListener listener) {
        this.node = node;
        if (listener != null)
